...
std::chrono::duration<double> measure(int threads, int particles, std::shared_ptr<Swarm::ObjectiveFunction<float, 2>> fitness, const Swarm::ChaosMap<float, 2> &chaosMap)
{
    ...
    Swarm::Optimizers::SAOptimizer<T, dim> swarm(fitness, initial_best, a, b, nN, nC, chaosMap, max_iterations, 395.0f, 0.95f, "/work/u10768804/benchmark/positions_benchmark_SAopt_drop_dist_00_r50_p28_n16k.txt");

    Swarm::Point<T, dim> center(0.0f);
    T radius = 5.0f;
...
int main()
{
    ...
    std::ofstream benchmark_file("/work/u10768804/benchmark/benchmark_SAopt_drop_dist_00_r50_p28_n16k.json");
